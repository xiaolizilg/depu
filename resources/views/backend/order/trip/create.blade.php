    ...
    <div class="col-md-12">
        <div class="panel panel-inverse" data-sortable-id="form-validation">
            ...
            <div class="panel-body panel-form">
                <form method="POST" action="{{ route('backend.order.trip.store', $order) }}" class="form-horizontal form-bordered" data-parsley-validate="true">
                    {!! csrf_field() !!}
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6">
                            ...
                            @foreach(config('website.trip_infos') as $trip_info)
                                @if(in_array($loop->index, [2,3,4,10]) && (!in_array($loop->index, $order->trip->trip_infos->pluck('type')->all())))
                                <option value="{{ $loop->index }}">{{ $trip_info }}</option>
                                @endif
                            @endforeach
                            </select>
                        </div>
