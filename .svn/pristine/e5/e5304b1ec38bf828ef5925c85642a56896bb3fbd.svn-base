...
namespace App\Models\Doctor\Traits\Attribute;
...
trait DoctorAttribute
{
    ...
    {
        if ($this->trashed()) {
            return $this->getRestoreButtonAttribute('backend.hospital.doctor.restore').
                $this->getDeletePermanentlyButtonAttribute('backend.hospital.doctor.forcedelete');
        }

        return
            $this->getEditButtonAttribute('backend.hospital.doctor.edit').
            $this->getDeleteButtonAttribute('backend.hospital.doctor.destroy');
    }
