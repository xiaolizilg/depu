        ...
        <div class="panel panel-inverse" data-sortable-id="form-validation">
            ...
            <div class="panel-body bg-silver">
                <div data-scrollbar="true" data-height="225px">
                    <ul class="chats">
                        <li class="left">
                            <span class="date-time">{{ $comment->createded_at }}</span>
                            <a href="javascript:;" class="name">{{ $comment->user->name }}</a>
                            <a href="javascript:;" class="image"><img alt="" src="{{ $comment->user->avatar }}" /></a>
                            <div class="message">
                                {{ $comment->content }}
                            </div>
                        </li>
                        @foreach($comment->replies as $repliy)
                        <li class="right">
                            <span class="date-time">{{ $repliy->createded_at }}</span>
                            <a href="#" class="name"><span class="label label-primary">专属客服</span> 我</a>
                            <a href="javascript:;" class="image"><img alt="" src="{{ $repliy->user->avatar }}" /></a>
                            <div class="message">
                                {{ $repliy->content }}
                            </div>
                        </li>
            ...
            <div class="panel-footer">
                <form name="send_message_form" data-id="message-form" method="POST" action="{{ route('backend.comment.update', [$comment->id]) }}">
                    {!! csrf_field() !!}
                    {!! method_field('PUT') !!}
