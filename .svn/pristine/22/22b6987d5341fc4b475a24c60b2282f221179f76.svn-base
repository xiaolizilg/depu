    ...
    <div class="col-md-12">
        <div class="panel panel-inverse" data-sortable-id="form-validation">
            <div class="panel-heading">
                ...
                <h4 class="panel-title">编辑服务</h4>
            </div>
            <div class="panel-body panel-form">
                <form method="POST" action="{{ route('backend.content.service.update', $service->id) }}" class="form-horizontal form-bordered" data-parsley-validate="true">
                    {!! csrf_field() !!}
                    {!! method_field('PUT') !!}
                    <legend><h3 class="text-center">服务信息</h3></legend>
                    <div class="form-group">
                        <label class="control-label col-md-4 col-sm-4">封面 :</label>
                        <div class="col-md-6 col-sm-6">
                            <div id="avatar_box" class="m-b-10" style="width:200px">
                                <div class="profile-image">
                                    <img class="m-auto" src="{{ get_image($service->icon_id) }}" title="{{ get_image($service->icon_id) }}" height="100%" />
                                </div>
                            </div>
                            <input class="form-control" type="text" name="icon_id" id="icon_id" value="{{$service->icon_id}}" />
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-md-4 col-sm-4">名称 :</label>
                        <div class="col-md-6 col-sm-6">
                            <input class="form-control" type="text" name="title" data-parsley-trigger="change" data-parsley-maxlength="80" data-parsley-required="true" placeholder="名称" value="{{$service->title}}" />
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-md-4 col-sm-4">描述 :</label>
                        <div class="col-md-6 col-sm-6">
                            <input class="form-control" type="text" name="desc" data-parsley-trigger="change" data-parsley-required="true" data-parsley-maxlength="80" placeholder="描述" value="{{$service->desc}}" />
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-md-4 col-sm-4">简介 :</label>
                        <div class="col-md-6 col-sm-6">
                            <input class="form-control" type="text" name="info" data-parsley-trigger="change" data-parsley-required="true" data-parsley-maxlength="80" placeholder="简介" value="{{$service->info}}" />
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6">
                            ...
                            <a href="{{ route('backend.content.service.index') }}" class="btn btn-white">返回</a>
                        </div>
                    </div>
                </form>
            </div>
