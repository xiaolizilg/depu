...
namespace App\Models\Ad\Traits\Attribute;
...
trait AdAttribute
{
    ...
    {
        if ($this->trashed()) {
            return $this->getRestoreButtonAttribute('backend.content.ad.restore').
                $this->getDeletePermanentlyButtonAttribute('backend.content.ad.forcedelete');
        }

        return
            $this->getEditButtonAttribute('backend.content.ad.edit').
            $this->getDeleteButtonAttribute('backend.content.ad.destroy');
    }
