...
namespace App\Models\Version\Traits\Attribute;
...
trait VersionAttribute
{
    ...
    {
        return 0 == $this->platform ? '安卓' : 'IOS';
    }

    public function getAppUpdatedAtAttribute($date)
    {
    ...
    {
        if ($this->trashed()) {
            return $this->getRestoreButtonAttribute('backend.content.version.restore').
                $this->getDeletePermanentlyButtonAttribute('backend.content.version.forcedelete');
        }

        return
            $this->getEditButtonAttribute('backend.content.version.edit').
            $this->getDeleteButtonAttribute('backend.content.version.destroy');
    }
