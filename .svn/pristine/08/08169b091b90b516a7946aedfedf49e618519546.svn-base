...
namespace App\Models\Hotal\Traits\Attribute;

use App\Models\Service\Service;
use Carbon\Carbon as Carbon;

/**
 * Class HotalAttribute.
 */
trait HotalAttribute
{
    ...
    {
        if ($this->trashed()) {
            return $this->getRestoreButtonAttribute('backend.hospital.hotal.restore').
                $this->getDeletePermanentlyButtonAttribute('backend.hospital.hotal.forcedelete');
        }

        return
            $this->getEditButtonAttribute('backend.hospital.hotal.edit').
            $this->getDeleteButtonAttribute('backend.hospital.hotal.destroy');
    }
