...
namespace App\Models\Trip\Traits\Attribute;

use Carbon\Carbon as Carbon;

/**
 * Class TripInfoAttribute.
 */
trait TripInfoAttribute
{
    ...
    {
        if (10 == $this->type && 0 != $this->tourism_id) {
            return $this->tourism->spots()->get(['title', 'cover_id', 'info', 'image_ids'])->map(function($item){
    ...
    {
        return config('website.trip_infos.' . $this->type, '');
    }
    ...
    public function getStatusDescAttribute()
    {
        ...
        switch ($this->attributes['status']) {
            case 0:
                ...
                break;
            case 1:
                $desc = '<span class="text-success"><i class="fa fa-check-circle"></i> 已确认</span>';
                break;
            case 2:
                $desc = '<span class="text-danger"><i class="fa fa-exclamation-circle"></i> 未确认</span>';
                break;
        }
    ...
    {
        return
            $this->getEditButtonAttribute('backend.order.trip.edit').
            $this->getStatusButtonAttribute('backend.order.trip.mark').
            $this->getDeleteButtonAttribute('backend.order.trip.destroy');
    }
